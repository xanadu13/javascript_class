...
<aside id="bo_vc_w" class="bo_vc_w">
	...
	<div id=onionButton style="color:#c00000;cursor:pointer;font-weight:bold;padding:10px;box-sizing:border-box;border:1px solid #cccccc;border-top:none;border-radius:5px;background-color:#eeeeee;text-align:center">[ 이모티콘 열기 ]</div>
	<div id=onionDiv style="padding:10px 0px 10px;box-sizing:border-box;border:1px solid #cccccc;border-top:none;border-radius:5px;display:none">
		<?php
		$onion_image = "";
		$onion_list = opendir(G5_PATH."/bbs/onion");
		while ($onion_file = readdir($onion_list)) if ($onion_file != "." && $onion_file != "..") $onion_image = $onion_image."'$onion_file',";
		closedir($onion_list);
		?>
		<script>
		onionButton.onclick = function() {
			if (this.innerHTML == "[ 이모티콘 열기 ]") {
				this.innerHTML = "[ 이모티콘 닫기 ]";
				onionDiv.style.display = "block";
				onionDiv.style.paddingLeft = (onionDiv.clientWidth - Math.floor(onionDiv.clientWidth / 50) * 50) / 2 + "px";
			}
			else {
				this.innerHTML = "[ 이모티콘 열기 ]";
				onionDiv.style.display = "none";
			}
		}
		onionGroup =[<?php echo substr($onion_image, 0, -1); ?>];
		onionGroup.sort();
		for (onion in onionGroup) {
			document.write("<img id=onion_" + onion + " style=display:inline-block;cursor:pointer src=<?php echo G5_URL; ?>/bbs/onion/" + onionGroup[onion] + ">");
			this["onion_" + onion].onclick = function() {
